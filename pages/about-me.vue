<template>
  <main v-if="!loading" id="about-me" class="page">
    ...
    <div id="page-menu" class="w-full flex">
      ...
      <div id="sections">
        <div id="section-icon" v-for="section in config.public.dev.about.sections" :key="section.title" :class="{ active: isSectionActive(section.title)}">
      ...
      <div id="section-content" class="hidden lg:block w-full h-full border-right">
        ...
        <div id="section-content-title" class="hidden lg:flex items-center min-w-full">
          ...
          <p v-html="config.public.dev.about.sections[currentSection].title" class="font-fira_regular text-white text-sm"></p>
        </div>

        <!-- folders -->
        <div>
          <div v-for="(folder, key, index) in config.public.dev.about.sections[currentSection].info" :key="key" class="grid grid-cols-2 items-center my-2 font-fira_regular text-menu-text" @click="focusCurrentFolder(folder)">
        ...
        <div id="section-content-title-contact" class="flex items-center min-w-full border-top">
          <img id="section-arrow-menu" src="/icons/arrow.svg" alt="" class="section-arrow mx-3 open">
          <p v-html="config.public.dev.contacts.direct.title" class="font-fira_regular text-white text-sm"></p>
        </div>
        <div id="contact-sources" class="hidden lg:flex lg:flex-col my-2">
          <div v-for="(source, key) in config.public.dev.contacts.direct.sources" :key="key" class="flex items-center mb-2">
      ...
      <div id="section-content" class="lg:hidden w-full font-fira_regular">
        ...
        <div v-for="section in config.public.dev.about.sections" :key="section.title">
          ...
          <div :id="'folders-' + section.title" class="hidden"> <!-- <div :id="'folders-' + section.title" :class="currentSection == section.title ? 'block' : 'hidden'"> -->
            <div v-for="(folder, key, index) in config.public.dev.about.sections[section.title].info" :key="key" class="grid grid-cols-2 items-center my-2 font-fira_regular text-menu-text hover:text-white hover:cursor-pointer" @click="focusCurrentFolder(folder)">
        ...
        <div id="contacts" class="hidden">
          <div v-for="(source, key) in config.public.dev.contacts.direct.sources" :key="key" class="flex items-center my-2">
      ...
      <div id="left" class="w-full flex flex-col border-right">
        ...
        <div class="tab-height w-full hidden lg:flex border-bot items-center">
          <div class="flex items-center border-right h-full">
            <p v-html="config.public.dev.about.sections[currentSection].title" class="font-fira_regular text-menu-text text-sm px-3"></p>
        ...
        <div id="tab-mobile" class="flex lg:hidden font-fira_retina">
            ...
            <h3 v-html="config.public.dev.about.sections[currentSection].title" class="text-white px-2"></h3>
            <span class="text-menu-text"> / </span>
            <h3 v-html="config.public.dev.about.sections[currentSection].info[folder].title" class="text-menu-text pl-2"></h3>
        </div>
        ...
        <div id="commented-text" class="flex h-full w-full lg:border-right overflow-hidden">
          ...
          <div class="w-full h-full ml-5 mr-10 lg:my-5 overflow-scroll">
              <CommentedText :text="config.public.dev.about.sections[currentSection].info[folder].description" />
          </div>
...
export default {
  ...
  methods: {
    ...
    focusCurrentFolder(folder) {
      ...
      this.currentSection = this.config.public.dev.about.sections[this.currentSection].info[folder.title] ? this.currentSection : Object.keys(this.config.public.dev.about.sections).find(section => this.config.public.dev.about.sections[section].info[folder.title])
    },
    /**
